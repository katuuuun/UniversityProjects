...
_c_int00:
	...
	stm 	#60-1, AR5			; number of gar
	stm 	#1-1, AR6			; start gar
loop_signal:
...
loop_sin:	
	...
	add		#1, A				; inc current gar
	addm    #1, gar				; inc current gar in mem
...
loop_for_1_gar:		
	rpt		#120-1
	ld		*AR2+, 16,	 A		; skip first period
	
	stm 	#N1,		 AR3	; leng 1 gar to AR3
	st		#0, 		 sum_sq	; clear
	
power_calc:
	ld		*AR2+, 16,	 A		; A = signal(120+i)
	squr    A, 			 A		; A = A^2
	ld		div_con,     T      ; T = 2/120
	mpya	A					; A = T * sth(A) = A/60
	dadd	sum_sq,		 A		; A = A + sum_sq
	dst		A,           sum_sq	; save new sum_sq
	banz	power_calc,  *AR3-

	sth     A,           *AR1+	; save to freq_resp
	banz	loop_for_1_gar, *AR4-
...
sect_proc:
	...
	macr 	*AR4+%, A			; A = A + T * a1_i
	ltd 	xn					; T = x(n); x(n-1) = x(n)
	macr 	*AR4+%, A			; A = A + T * a0_i
	ltd 	yn2					; T = y(n-2); skip2 = y(n-2)
	masr 	*AR4+%, A			; A = A - T * b2_i
	ltd 	yn1					; T = y(n-1); y(n-2) = y(n-1)
	macr 	*AR4+%, A			; A = A - T * b1_i
	sth 	A, 		yn			; y(n) = A	
	delay 	yn     				; y(n-1) = y(n)
	sth 	A, 		*AR2+		; signal = A
...
RV	.bes	32*16				; stack
	...
	.align
gar		.word	1				; N gar
N1   	.set	120-1			; length 1 gar		
N   	.set	14400-1			; length all gar
div_con	.word   222h			; 546 = 2/120 (energy divider)
...
Cn		.word	7FFFh			; Cn = 7FFF because cos(0)?
	...
	.align 2
sum_sq		.word 0,0			; 32-bit
freq_resp	.word 0				; current element freq response
res			.space 60*16		; result freq response
